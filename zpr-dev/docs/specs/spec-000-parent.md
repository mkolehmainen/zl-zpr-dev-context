...
A dedicated Git repository named `zl-zpr-dev-context` SHALL contain shared ZPR development context.
...
`zl-zpr-dev-context` is the canonical source for:
...
```text
~/src/zl_zpr/
├── context/             # clone of zl-zpr-dev-context
├── node/
├── visa-service/
├── zpl/
├── router/
...
The exact repository set and checkout names SHALL be defined by `workspace.yaml` in `zl-zpr-dev-context`.
...
<!-- Source: zl-zpr-dev-context commit 4ba137c -->
```
...
```bash
zpr-dev setup \
    --workspace ~/src/zl_zpr \
    --context-url git@github.com:example/zl-zpr-dev-context.git
```

`setup` SHALL:

1. create the workspace directory if necessary;
2. clone `zl-zpr-dev-context` into `<workspace>/context`;
...
This is useful after editing the local `zl-zpr-dev-context` checkout.

Example workflow:

```bash
cd ~/src/zl_zpr/context
vim AGENTS.md
git diff
...
```text
/home/alice/src/zl_zpr/context/docs/VISA_SERVICE.md
```
...
skills:
  external_dirs:
    - /home/alice/src/zl_zpr/context/skills
```
...
<!-- Shared context: zl-zpr-dev-context @ 650ad18 -->
...
Not all documentation belongs in `zl-zpr-dev-context`.

A useful rule is:

```text
Cross-repository architectural knowledge
    -> zl-zpr-dev-context/docs
...
```text
zl-zpr-dev-context/docs/ZDP.md
```
...
The `zl-zpr-dev-context` repository SHALL NOT contain:
...
### 14.2 Open Question 2: Where should `zl-zpr-dev-context` live?

Possible locations include:

```text
~/src/zl_zpr/context
~/src/zl_zpr/zl-zpr-dev-context
```

`context` is shorter and makes the workspace easier to navigate.

`zl-zpr-dev-context` is more explicit and matches the repository name.

**ANSWER:** `zl-zpr-dev-context`. Use repository name.
...
1. Use `zl-zpr-dev-context` as the single canonical repository for shared development knowledge.
2. Use `~/src/zl_zpr` as the default workspace.
3. Check out the context repository as `~/src/zl_zpr/zl-zpr-dev-context`.
4. Keep detailed documentation under `zl-zpr-dev-context/docs/`.
5. Keep shared agent skills under `zl-zpr-dev-context/skills/`.
