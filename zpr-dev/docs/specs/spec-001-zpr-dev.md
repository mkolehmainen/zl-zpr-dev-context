...
`--force` (§5.1) inverts the last two points for a workspace that has decided
the shared context wins. Classification is unchanged — `plan` still returns
`Foreign`, because that is a fact about the file — but:

- `apply` **writes** the file, prints `overwriting <path>: not generated by
  zpr-dev (--force)` regardless of `--verbose`, and counts it in both
  `ApplySummary.forced` and `ApplySummary.updated`.
- `validate` raises a **warning** per file rather than an error, and exits zero:
  `sync --force` *can* clear the finding, so it is drift, not a blocker.

The distinction the flag preserves is that the clobber is always announced. It
suppresses the error, never the notice.
...
--force               Overwrite generated files that zpr-dev did not write
-h, --help
--version
```
...
`--force` narrows to exactly one guard: the refusal to overwrite a file
`zpr-dev` did not generate (§4.6). It is not a general "ignore errors" switch,
and it does not combine with `--dry-run` to write anything. Use it in a
workspace where the shared context is authoritative and a repository's own
`AGENTS.md` is expected to be replaced; the correct fix in the repository
remains renaming that file to `AGENTS.repo.md`.
...
| `--force sync` over a hand-written `AGENTS.md` | File is overwritten; `overwriting ...` printed |
| `--force validate` over a hand-written `AGENTS.md` | Warning, not error; exits zero |
