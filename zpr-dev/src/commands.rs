...
fn generate_context(ctx: &Ctx, manifest: &Manifest) -> Result<()> {
    ...
    // `apply` has already named each file it skipped or clobbered; these are the
    // tallies, so neither can be lost in the scroll of a large workspace.
    ...
    if summary.forced > 0 {
        report(
            ctx,
            format!(
                "warning: overwrote {} file{} not generated by zpr-dev (--force)",
                summary.forced,
                plural(summary.forced)
            ),
        );
    }
    Ok(())
...
fn check_shared_context(ctx: &Ctx, manifest: &Manifest, report: &mut Report) {
    ...
    match generate::plan(ctx, manifest) {
        Ok(plans) => {
            // A foreign file is normally an error rather than drift: plain
            // `sync` cannot clear it, so it needs a human to rename or delete
            // the file. Under `--force` `sync` *can* clear it — by clobbering
            // it — so the finding drops to a warning and validation passes.
            // Named per file either way, because the fix is per file.
            for plan in &plans {
                for file in &plan.files {
                    ...
                    if ctx.force {
                        report.warn(format!(
                            "{}: {} was not generated by zpr-dev; --force will overwrite it",
                            plan.name,
                            file.path.display()
                        ));
                    } else {
